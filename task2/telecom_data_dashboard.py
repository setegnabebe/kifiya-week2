...
# Configure Streamlit
st.set_page_config(page_title="Telecom User Engagement Dashboard", layout="wide")
...
@st.cache_data
def fetch_telecom_data(_engine):  # Updated function signature with a leading underscore for 'engine'
    try:
        query = """
        SELECT 
        ...
        """
        data = pd.read_sql(query, _engine)  # Pass _engine (with underscore) instead of 'engine'
        return data
    except Exception as e:
...
def main():
    ...
    # Fetch data
    telecom_data = fetch_telecom_data(engine)  # Use 'engine' here
    ...
    # Top 10 Users by Application Traffic
    st.subheader("Top 10 Users by Application Traffic")
